...
pub const ZnPidCtrl = struct {
    ...
    pAvarage: f32 = 0, // Avarage Kp (Proportional gain)
    iAvarage: f32 = 0, // Avarage Ti (Integral gain)
    dAvarage: f32 = 0, // Avarage Td (Derivative gain)
    ...
    pub fn runCalibration(self: *Self) void {
        self.pError = 0;
        self.iError = 0;
        self.dError = 0;
    ...
    pub fn getAvarageGains(self: *Self) ZnGains {
        if (self.index < 2) {
            return self.getGains();
        }

        return ZnGains{
            .Kp = (self.pAvarage / @as(f32, @floatFromInt(self.index - 1))),
            .Ti = (self.iAvarage / @as(f32, @floatFromInt(self.index - 1))),
            .Td = (self.dAvarage / @as(f32, @floatFromInt(self.index - 1))),
        };
    }
    ...
    pub fn getPid(
        ...
        minBangBang: u32, // Min threshold value for bang-bang control (not used in during calibration) https://en.wikipedia.org/wiki/Bang%E2%80%93bang_control or min level for triggerin calibration (iterations must be greater than 0)
        maxBangBang: u32, // Max threshold value for bang-bang control (not used in during calibration) https://en.wikipedia.org/wiki/Bang%E2%80%93bang_control or max level for triggerin calibration (iterations must be greater than 0)
        autoCalibrate: f32,
    ) f32 {
        if (self.calibrate and self.index >= self.iterations) {
        ...
        if (autoCalibrate > 0 and ((setPoint - input) > autoCalibrate or (input - setPoint) > autoCalibrate)) {
            self.runCalibration();
            return maxOutput;
        }
...
fn setOutput(input: f32, speed: f32, friction: f32) f32 {
    ...
    if (speed >= 50) {
        ret += (0.001337 + ((reng.random().float(f32) / 10000) * friction)) * speed;
    } else {
        ret -= ((0.001337 + ((reng.random().float(f32) / 10000) * friction)) * 5) * ((50 - speed) + 1);
    }
...
test "ZnSafe - With calibration" {
    ...
    for (0..100000000) |us| { // Every iteration is happening in 100ms cycles
        if (!ctrl.isCalibrating()) {
            if ((setpoint - input) > 0.99 or (input - setpoint) > 0.99) {
                ctrl.runCalibration();
                calibrated += 1;
            } else {
                ctrl.setGains(ctrl.getAvarageGains());
            }
        }

        speed = ctrl.getPid((us * 100000), input, setpoint, 0, 100, 0, 0, 0);
    ...
    std.debug.print("{any}\n", .{ctrl.getAvarageGains()});
    std.debug.print("Calibrated {d} times.\n\n", .{calibrated});
}

test "Static ZnGains - Without calibration" {
    var ctrl = ZnPidCtrl.init(.{ .Kp = 7.08424133e+02, .Ti = 3.99408843e+02, .Td = 8.47085388e+02 }, 0);
    var input: f32 = getInput();
    var speed: f32 = 0;
    const setpoint = 668.5;
...
test "ZnSafe - With auto calibration" {
    ...
    for (0..10000000) |us| { // Every iteration is happening in 100ms cycles
        speed = ctrl.getPid((us * 100000), input, setpoint, 0, 100, 0, 0, 0.01);
        input = setOutput(input, speed, 25);
